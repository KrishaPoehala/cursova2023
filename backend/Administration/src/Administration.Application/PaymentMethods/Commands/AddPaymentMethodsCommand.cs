...
public class AddPaymentMethodsCommandHandler : IRequestHandler<AddPaymentMethodsCommand>
{
    ...
    public AddPaymentMethodsCommandHandler(IApplicationDbContext context, IMapper mapper,
        ICurrentUserService userService, PlaidClient plaidClient, IConfiguration conf)
    {
        _context = context;
    ...
    {
        var exchangedToken = await _plaidClient.ItemPublicTokenExchangeAsync(new ItemPublicTokenExchangeRequest()
        {
            PublicToken = request.PublicToken,
            ClientId = _conf["Plaid:ClientId"],
            Secret = _conf["Plaid:Secret"],
        });
